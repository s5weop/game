...
class MainMenu(BoxLayout):
    def __init__(self, **kwargs):
        ...
        # Кнопка "Старт"
        self.start_button = Button(text='Старт', size_hint=(1, 0.1))
...
class GameScreen(BoxLayout):
    def __init__(self, is_winner, **kwargs):
        ...
        # Если игрок проиграл, добавляем кнопку "Начать уровень заново"
        if not is_winner:
            self.restart_button = Button(text='Начать уровень заново', size_hint=(1, 0.2))
            self.restart_button.bind(on_press=self.restart_level)
